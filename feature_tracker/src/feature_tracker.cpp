...
void FeatureTracker::readImage(const cv::Mat &_img, double _cur_time)
{
    ...
    forw_pts.clear();
     ...
     //如果不是第一帧，即前一帧已经有提取（跟踪）的特征点时（cur_pts.size()>0），forw_img相对于cur_img进行光流跟踪
    if (cur_pts.size() > 0)
    {
        TicToc t_o;
        vector<uchar> status;
    ...
    if (PUB_THIS_FRAME)
    {
        //通过基本矩阵剔除outliers。这里通过cur_point,forw_point来计算un_cur_points和un_forw_points，进而计算F
        rejectWithF();
        ...
        //通过setMask， forw_pts ,ids, track_cnt中元素的顺序已经按照跟踪次数track_cnt由大到小进行了排列
...
void FeatureTracker::undistortedPoints()
{
    ...
    //！！！！！！调用该函数的时候，cur_pts已经被赋值为forw_pts！！！！！！！
    for (unsigned int i = 0; i < cur_pts.size(); i++)
    {
    ...
    if (!prev_un_pts_map.empty())
    {
        ...
        {
            if (ids[i] != -1) //ids[i]!=-1说明不是新提取的点，而是从上一帧跟踪上的点
            {
    ...
    else  //如果没有prev_un_pts,则将cur_pts对应的点的速度设为0
    {
        for (unsigned int i = 0; i < cur_pts.size(); i++)
        {
            pts_velocity.push_back(cv::Point2f(0, 0));
