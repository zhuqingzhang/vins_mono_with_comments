...
{
    //nav_msgs::Path 包含header和 poses(position.orientation)
...
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
    ...
    //建一个新的图像序列
    //sequence_cnt从0开始，cur_kf->sequence从1开始
    ...
    cur_kf->getVioPose(vio_P_cur, vio_R_cur); //当前帧在w下的位姿 T_w_i
    ...
    {
        TicToc tmp_t;
        //回环检测，返回回环候选帧的索引，该索引是与cur_kf相似度评分高于0.015的最早的关键帧的索引
        loop_index = detectLoop(cur_kf, cur_kf->index);
    }
    else
    {
        //将cur_kf的图像放入image_pool中，同时将cur_kf的描述子加入db中
	...
	if (loop_index != -1)
	{
        ...
        //当前帧与回环候选帧进行描述子匹配
        if (cur_kf->findConnection(old_kf))//与old_kf进行特征点匹配，匹配数量大于一定的阈值，计算两帧相对的位姿信息等，
                                           // 发布match_point的消息给estimator_node
        {
            //如果当前的回环帧loop_index比之前匹配到的loop_index都早，则更新earliest_loop_index
            if (earliest_loop_index > loop_index || earliest_loop_index == -1)
                earliest_loop_index = loop_index;

            Vector3d w_P_old, w_P_cur, vio_P_cur;
            Matrix3d w_R_old, w_R_cur, vio_R_cur;
            old_kf->getVioPose(w_P_old, w_R_old); //T_w_i
            ...
            Matrix3d shift_r;
            Vector3d shift_t; 
            shift_yaw = Utility::R2ypr(w_R_cur).x() - Utility::R2ypr(vio_R_cur).x();//通过旋转矩阵计算yaw的偏移量
            shift_r = Utility::ypr2R(Vector3d(shift_yaw, 0, 0));//认为只有yaw会偏移（因为不可观），以此来求旋转矩阵的偏移量
            //注意这块的计算！！！！
...
//回环检测，找到与keyframe相似度评分大于0.015的最早的关键帧
int PoseGraph::detectLoop(KeyFrame* keyframe, int frame_index)
{
    ...
    //确保与相邻帧具有好的相似度评分
    //按理说这块做的事是将相似度好的图像显示出来。然而上一步已经将ret中所有的图像都显示出来了，这块没啥用
    //仅仅将find_loop置为true（当有相似度较好的相邻帧时）
...
void PoseGraph::optimize4DoF()
{
    while(true)
    {
        ...
        //取出最新一个待优化帧作为当前帧
        m_optimize_buf.lock();
        while(!optimize_buf.empty()) //得到cur_index 和与之相匹配的回环帧 earliest_loop_index
        {
        ...
        if (cur_index != -1)
        {
            ...
            for (it = keyframelist.begin(); it != keyframelist.end(); it++)
            {
                ...
                //add sequential edge
                for (int j = 1; j < 5; j++)//寻找与(*it)->local_index，即当前迭代的keyframe相邻的关键帧,最多找4个。
                {
                ...
                if((*it)->has_loop)
                {
                    ...
                    int connected_index = getKeyFrame((*it)->loop_index)->local_index;  //当前帧(*it)的闭环帧对应的local_id
                    Vector3d euler_conncected = Utility::R2ypr(q_array[connected_index].toRotationMatrix());
                    Vector3d relative_t;
                    relative_t = (*it)->getLoopRelativeT(); //当前帧(*it)与其闭环帧的相对位姿
                    double relative_yaw = (*it)->getLoopRelativeYaw();
                    //FourDOFWeightError给yaw角的差值乘上了1/10
            ...
            */
            ...
            it++; //cur_kf的下一帧，通过前边算出的r_drift和m_drift来更新剩下的keyframe
            for (; it != keyframelist.end(); it++)
            {
                Vector3d P;
                Matrix3d R;
