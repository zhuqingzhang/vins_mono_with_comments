		           ...
		           vector<double> &_point_id, int _sequence)
{
	...
	sequence = _sequence;
	computeWindowBRIEFPoint();//计算point_2d_uv的表述子
	computeBRIEFPoint();//在image中提取新的特征点，并计算描述子，然后将新提取的这些特征点去畸变
	if(!DEBUG_IMAGE)
		image.release();
}
...
void KeyFrame::computeWindowBRIEFPoint()
{
	...
	extractor(image, window_keypoints, window_brief_descriptors); //对image中的window_keypoints提取描述子存于window_brief_descriptors
}
...
void KeyFrame::computeBRIEFPoint()
{
	...
	for (int i = 0; i < (int)keypoints.size(); i++)
	{
		...
		//得到的tmp_p是去畸变后在单位球上的坐标
		cv::KeyPoint tmp_norm;
		tmp_norm.pt = cv::Point2f(tmp_p.x()/tmp_p.z(), tmp_p.y()/tmp_p.z()); //转换到归一化相机坐标系下
...
//关键帧中“某个特征点”的描述子 与回环帧的“所有”描述子匹配，在searchByBRIEFDes中被调用
...
/**
 * @brief   将关键帧的所有keypoints与回环帧进行BRIEF描述子匹配，调用searchInAera
...
//用origin_vio_R,T的数据做PNP进行优化，优化后的位姿存在PnP_T_old,PnP_R_old中
                         ...
                         Eigen::Vector3d &PnP_T_old, Eigen::Matrix3d &PnP_R_old)
{
    ...
    else
    {
        if (CV_MINOR_VERSION < 2)
        	...
        	//mathced3d为特征点的在世界坐标系下3d坐标，matched_2d_old_norm为回环帧观测到的像素坐标
        	//得到更新后的rvec和t，这个位姿是回环帧的位姿
    ...
    PnP_T_old = T_w_c_old - PnP_R_old * tic;  //修正之后的当前帧的位姿

}

//寻找并建立关键帧与回环帧之间的匹配关系
//1、通过描述子匹配   2、通过fundmental矩阵取出outlier(被注释掉了) 3、通过PnP去除外点
bool KeyFrame::findConnection(KeyFrame* old_kf)
{
	...
	//this Keyframe为当前处理的关键帧， old_kf为要进行匹配的回环帧
	searchByBRIEFDes(matched_2d_old, matched_2d_old_norm, status, old_kf->brief_descriptors, old_kf->keypoints, old_kf->keypoints_norm);
	//上边这个函数进行完，matched_2d_old中存放着与this Keyframe相匹配的old_kf中的特征点。
	// status的大小为this_Kayframe的特征点（window_keypoints）的数量，记录着每一个特征点是否与old_kf成功匹配

	//将为成功匹配的点从matched_*中剔除
