...
void process()
{
    ...
    while (true)
    {
        ...
        if (pose_msg != NULL)
        {
            ...
            //将距上一关键帧距离（平移向量的模）超过SKIP_DIS的图像创建为关键帧，0
...
int main(int argc, char **argv)
{
    ...
    if (LOOP_CLOSURE)
    {
        ...
        //读取字典
        std::string pkg_path = ros::package::getPath("pose_graph");//通过ros::package获取"pose_graph“文件夹的路径
    ...
    //发布的topic
    pub_match_img = n.advertise<sensor_msgs::Image>("match_image", 1000); //debug image时发布回环匹配的图片
    pub_camera_pose_visual = n.advertise<visualization_msgs::MarkerArray>("camera_pose_visual", 1000);
    pub_key_odometrys = n.advertise<visualization_msgs::Marker>("key_odometrys", 1000);
    pub_vio_path = n.advertise<nav_msgs::Path>("no_loop_path", 1000);
    pub_match_points = n.advertise<sensor_msgs::PointCloud>("match_points", 100); //只有fast_relocalization为1时才发布"match_points"的消息
