...
private:
	...
	void optimize4DoF();
	void updatePath();
	list<KeyFrame*> keyframelist; //在pose_graph_node中每次调用addKeyFrame时就会在这个容器中添加一个keyframe
 ...
 public:
  ...
  bool operator()(const T* theta_radians, const T* delta_theta_radians,
    ...
    *theta_radians_plus_delta =
        NormalizeAngle(*theta_radians + *delta_theta_radians); //角度在正负pi之间
  ...
  static ceres::LocalParameterization* Create() {
    return (new ceres::AutoDiffLocalParameterization<AngleLocalParameterization,
                                                     1, 1>);  //<加法准则（更新准则），global_size,local_size>
  }
};
