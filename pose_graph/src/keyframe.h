...
public:
	...
	vector<cv::KeyPoint> keypoints; //未去畸变的特征点---------------
	                               //                             |-->对this_Keyframe新提取的特征点
	vector<cv::KeyPoint> keypoints_norm; //归一化相机坐标系下的坐标---
	vector<cv::KeyPoint> window_keypoints;//与point_2d_uv对应，为this_Keyframe传入进来时已有的特征点
	...
	bool has_loop;
	int loop_index;
	Eigen::Matrix<double, 8, 1 > loop_info; //存放着this_keyframe与id 为loop_index的keyframe的相对位姿。 T_loop_this
	//注意，这块相对位姿是thiskeyframe相对于solvePnP优化后求得的loopframe的位姿，而loopframe本身存储的位姿并没有更新为solvePnP优化后的位姿
	// x,y,z,qw,qx,qy,qz,yaw
};
