...
{
    liftProjective(p, P); //得到的P是p去畸变后的二维图像坐标+ z=1

    P.normalize();
    //得到的是在unit sphere上的3维坐标
}
...
PinholeCamera::liftProjective(const Eigen::Vector2d& p, Eigen::Vector3d& P) const
{
    ...
    else
    {
        ...
        else
        {
            ...
            int n = 8;
            Eigen::Vector2d d_u;
            distortion(Eigen::Vector2d(mx_d, my_d), d_u); //输出的du是对传入的(mx_d,my_d)进行畸变后的坐标偏移
            //所以，可以这么认为： distort_pt = undistort_pt+du --> undistort_pt=distort_pt-du
            //在不知道undistort_pt时，可以将(mx_d,my_d)作为初始值，计算畸变偏移量du，然后在(mx_d,my_d)的基础上减掉du
            //得到一个更优的点，然后进行迭代，使值最终收敛。
            // Approximate value
            mx_u = mx_d - d_u(0);
            my_u = my_d - d_u(1);
