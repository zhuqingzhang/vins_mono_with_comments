...
void solveGyroscopeBias(map<double, ImageFrame> &all_image_frame, Vector3d* Bgs)
{
    ...
    for (frame_i = all_image_frame.begin(); next(frame_i) != all_image_frame.end(); frame_i++)
    {
        ...
        //tmp_A = J_j_bw
        //别忘了frame的id和pre_integration相差了1. 所以frame_j对应的预积分实际上是frame_i到frame_j之间的预积分。
        tmp_A = frame_j->second.pre_integration->jacobian.template block<3, 3>(O_R, O_BG);
        //tmp_b = 2 * (r^bk_bk+1)^-1 * (q^c0_bk)^-1 * (q^c0_bk+1)
        //      = 2 * (r^bk_bk+1)^-1 * q_ij
        tmp_b = 2 * (frame_j->second.pre_integration->delta_q.inverse() * q_ij).vec();

        /* q_ij=gamma(即delta_q，预积分)*[1, 0.5*J*delta_bg]
         * -->  gamma^-1*q_ij=[1,0.5*J*delta_bg]
         * -->  2*(gamma^-1*q_ij).vec()=J*delta_bg
...
{
    //g0 = (g^-)*||g||
    Vector3d g0 = g.normalized() * G.norm();//normalized是进行单位化， norm是计算向量的大小
    Vector3d lx, ly;
    //VectorXd x;
    ...
    for(int k = 0; k < 4; k++)//迭代4次
    {
        ...
        for (frame_i = all_image_frame.begin(); next(frame_i) != all_image_frame.end(); frame_i++, i++)
        {
            ...
            MatrixXd tmp_A(6, 9);  //9：速度v（2*3） lxly方向的关于g的扰动大小（2*1） scale（1）
            tmp_A.setZero();
            VectorXd tmp_b(6);
            tmp_b.setZero();
...
bool LinearAlignment(map<double, ImageFrame> &all_image_frame, Vector3d &g, VectorXd &x)
{
    int all_frame_count = all_image_frame.size();
    //优化量x的总维度
    int n_state = all_frame_count * 3 + 3 + 1; //v,g,s
    ...
    for (frame_i = all_image_frame.begin(); next(frame_i) != all_image_frame.end(); frame_i++, i++)
    {
        ...
        MatrixXd tmp_A(6, 10);  //6对应预积分alpha，beta,10对应两个相邻帧的速度v(2*3)+g(3)+s(1)=10
        tmp_A.setZero();
        VectorXd tmp_b(6);
        tmp_b.setZero();
        ...
        //关于scale的系数缩小了100倍，导致计算得到的scale被放大了100倍，最后需要再除以100
