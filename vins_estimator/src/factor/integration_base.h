...
class IntegrationBase
{
  public:
...

    {
        ...
        noise.block<3, 3>(0, 0) =  (ACC_N * ACC_N) * Eigen::Matrix3d::Identity();  //ba_k的高斯噪声协方差
        noise.block<3, 3>(3, 3) =  (GYR_N * GYR_N) * Eigen::Matrix3d::Identity();  //bg_k的高斯噪声协方差
        noise.block<3, 3>(6, 6) =  (ACC_N * ACC_N) * Eigen::Matrix3d::Identity();  //ba_k+1的高斯噪声协方差
        noise.block<3, 3>(9, 9) =  (GYR_N * GYR_N) * Eigen::Matrix3d::Identity();  //bg_k+1的高斯噪声协方差
        noise.block<3, 3>(12, 12) =  (ACC_W * ACC_W) * Eigen::Matrix3d::Identity(); //ba_k的随机游走噪声协方差
        noise.block<3, 3>(15, 15) =  (GYR_W * GYR_W) * Eigen::Matrix3d::Identity(); //bg_k的随机游走噪声协方差
    }
    ...
    */
    void propagate(double _dt, const Eigen::Vector3d &_acc_1, const Eigen::Vector3d &_gyr_1)
    {
        ...
        //acc_0,gyro_0为k时刻的，acc_1,gro_1为k+1时刻的
                                          ...
                                          const Eigen::Vector3d &Pj, const Eigen::Quaterniond &Qj, const Eigen::Vector3d &Vj, const Eigen::Vector3d &Baj, const Eigen::Vector3d &Bgj)
    {
        ...
        //更新后的bias的变化量。注意此处用的是i时刻的。因为linearized_ba/bg就是i时刻对应的bias
    ...
    double dt;
    Eigen::Vector3d acc_0, gyr_0; //k时刻的的线加速度和角速度
    Eigen::Vector3d acc_1, gyr_1; //k+1时刻的线加速度和角速度
    ...
    double sum_dt;//两个图像帧之间的时间差，是由每帧imu之间的时间差dt累加起来的
    //预积分值，对应于alpha，gamma，beta
...
/*
                            ...
                            const Eigen::Vector3d &linearized_ba, const Eigen::Vector3d &linearized_bg)
    {
        ...
        如果jacobian计算正确，p diff和p jacob diff应该近似相等
