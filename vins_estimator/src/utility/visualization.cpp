...
{
    //由imu递推得到的P->odometry.pose.position Q->odometry.pose.oritentation V->odometry.twist
...
void pubKeyframe(const Estimator &estimator)
{
    ...
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR && estimator.marginalization_flag == 0)//边缘化最老帧
    {
        int i = WINDOW_SIZE - 2;
        //Vector3d P = estimator.Ps[i] + estimator.Rs[i] * estimator.tic[0];
        //边缘化前的次新帧
